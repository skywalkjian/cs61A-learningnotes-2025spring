...
def accuracy(typed, source):
    ...
    if len(typed_words)==0:
        if len(source_words)==0:
            ...
            return 0.0
    return son*100/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    LIMIT=diff_function(typed_word,word_list[0],limit)
    index=-1
    ans_index=0
    for i in word_list:
        index+=1
        if diff_function(typed_word,i,limit)<LIMIT:
            LIMIT=diff_function(typed_word,i,limit)
            ans_index=index
    if LIMIT>limit:
        return typed_word
    else:
        return word_list[ans_index]
